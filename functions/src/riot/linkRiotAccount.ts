    ...
    try {
      ...
      if (linkedAccountDoc.exists) {
        ...
        if (linkedData && linkedData.userId !== userId) {
          ...
          throw new HttpsError(
            "already-exists",
            "This Riot account is already linked to another RankdUp profile. Please unlink it from the other profile first, or contact support if you believe this is an error."
          );
        }
